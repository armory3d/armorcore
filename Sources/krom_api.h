...
void krom_g4_set_vertex_buffers(vertex_buffer_t_array_t *vertex_buffers) {
...
void krom_g4_set_mipmaps(kinc_g4_texture_t *texture, image_t_array_t *mipmaps) {
	for (int32_t i = 0; i < mipmaps->length; ++i) {
		kinc_g4_texture_t *mipmapobj = mipmaps->buffer[i]->texture_;
...
void krom_g4_begin(image_t *render_target, image_t_array_t *additional) {
	if (render_target == NULL) {
		...
		if (additional != NULL) {
			...
			for (int32_t i = 1; i < length; ++i) {
				kinc_g4_render_target_t *art = (kinc_g4_render_target_t *)additional->buffer[i - 1]->render_target_;
...
#ifdef WITH_NFD
string_t_array_t *krom_open_dialog(char *filter_list, char *default_path, bool open_multiple) {
	...
	if (result == NFD_OKAY) {
		...
		string_t_array_t *result = any_array_create(path_count);
