...
class ParticleSystem {
	...
	function updateGpu(object: MeshObject, owner: MeshObject) {
		if (!object.data.instanced) setupGeomGpu(object, owner);
	...
	function setupGeomGpu(object: MeshObject, owner: MeshObject) {
		...
		switch (r.emit_from) {
			...
			case 1: // Face
				var positions = owner.data.positions.values;

				for (p in particles) {
					// Choose random index array (there is one per material) and random face
					var ia = owner.data.indices[Std.random(owner.data.indices.length)];
