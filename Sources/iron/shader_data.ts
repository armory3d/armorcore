...
function shader_data_create(raw: shader_data_t, done: (sd: shader_data_t)=>void, override_context: shader_override_t = null) {
	...
	for (let i = 0; i < raw.contexts.length; ++i) {
		...
		shader_context_create(c, (con: shader_context_t) => {
			raw._contexts[i] = con;
			contexts_loaded++;
...
function shader_data_delete(raw: shader_data_t) {
	for (let c of raw._contexts) shader_context_delete(c);
}
...
function shader_context_compile(raw: shader_context_t, done: (sc: shader_context_t)=>void) {
	if (raw._pipe_state != null) pipeline_delete(raw._pipe_state);
	raw._pipe_state = pipeline_create();
	raw._constants = [];
	raw._tex_units = [];

	if (raw._instancing_type > 0) { // Instancing
		let instStruct = vertex_struct_create();
		vertex_struct_add(instStruct, "ipos", VertexData.F32_3X);
		if (raw._instancing_type == 2 || raw._instancing_type == 4) {
			vertex_struct_add(instStruct, "irot", VertexData.F32_3X);
		}
		if (raw._instancing_type == 3 || raw._instancing_type == 4) {
			vertex_struct_add(instStruct, "iscl", VertexData.F32_3X);
		}
		instStruct.instanced = true;
		raw._pipe_state.inputLayout = [raw._structure, instStruct];
	}
	else { // Regular
		raw._pipe_state.inputLayout = [raw._structure];
	}

	// Depth
	raw._pipe_state.depthWrite = raw.depth_write;
	raw._pipe_state.depthMode = shader_context_get_compare_mode(raw.compare_mode);

	// Cull
	raw._pipe_state.cullMode = shader_context_get_cull_mode(raw.cull_mode);

	// Blending
	if (raw.blend_source != null) raw._pipe_state.blendSource = shader_context_get_blend_fac(raw.blend_source);
	if (raw.blend_destination != null) raw._pipe_state.blendDestination = shader_context_get_blend_fac(raw.blend_destination);
	if (raw.alpha_blend_source != null) raw._pipe_state.alphaBlendSource = shader_context_get_blend_fac(raw.alpha_blend_source);
	if (raw.alpha_blend_destination != null) raw._pipe_state.alphaBlendDestination = shader_context_get_blend_fac(raw.alpha_blend_destination);

	// Per-target color write mask
	if (raw.color_writes_red != null) for (let i = 0; i < raw.color_writes_red.length; ++i) raw._pipe_state.colorWriteMasksRed[i] = raw.color_writes_red[i];
	if (raw.color_writes_green != null) for (let i = 0; i < raw.color_writes_green.length; ++i) raw._pipe_state.colorWriteMasksGreen[i] = raw.color_writes_green[i];
	if (raw.color_writes_blue != null) for (let i = 0; i < raw.color_writes_blue.length; ++i) raw._pipe_state.colorWriteMasksBlue[i] = raw.color_writes_blue[i];
	if (raw.color_writes_alpha != null) for (let i = 0; i < raw.color_writes_alpha.length; ++i) raw._pipe_state.colorWriteMasksAlpha[i] = raw.color_writes_alpha[i];

	// Color attachment format
	if (raw.color_attachments != null) {
		raw._pipe_state.colorAttachmentCount = raw.color_attachments.length;
		for (let i = 0; i < raw.color_attachments.length; ++i) raw._pipe_state.colorAttachments[i] = shader_context_get_tex_format(raw.color_attachments[i]);
	}

	// Depth attachment format
	if (raw.depth_attachment != null) {
		raw._pipe_state.depthStencilAttachment = shader_context_get_depth_format(raw.depth_attachment);
	}

	// Shaders
	if (raw.shader_from_source) {
		raw._pipe_state.vertexShader = shader_from_source(raw.vertex_shader, ShaderType.Vertex);
		raw._pipe_state.fragmentShader = shader_from_source(raw.fragment_shader, ShaderType.Fragment);

		// Shader compile error
		if (raw._pipe_state.vertexShader.shader_ == null || raw._pipe_state.fragmentShader.shader_ == null) {
			done(null);
			return;
		}
		...
		function load_shader(file: string, type: i32) {
			...
			data_get_blob(path, (b: ArrayBuffer) => {
				...
				shadersLoaded++;
				if (shadersLoaded >= numShaders) shader_context_finish_compile(raw, done);
			});
		}
		load_shader(raw.vertex_shader, 0);
		load_shader(raw.fragment_shader, 1);
		if (raw.geometry_shader != null) load_shader(raw.geometry_shader, 2);

		///else

		raw._pipe_state.fragmentShader = sys_get_shader(raw.fragment_shader);
		raw._pipe_state.vertexShader = sys_get_shader(raw.vertex_shader);
		if (raw.geometry_shader != null) {
			raw._pipe_state.geometryShader = sys_get_shader(raw.geometry_shader);
		}
		shader_context_finish_compile(raw, done);
...
function shader_context_finish_compile(raw: shader_context_t, done: (sc: shader_context_t)=>void) {
	...
	if (raw._override_context != null) {
		if (raw._override_context.cull_mode != null) {
			raw._pipe_state.cullMode = shader_context_get_cull_mode(raw._override_context.cull_mode);
		}
	}
	...
	if (raw.constants != null) {
		for (let c of raw.constants) shader_context_add_const(raw, c);
	}

	if (raw.texture_units != null) {
		for (let tu of raw.texture_units) shader_context_add_tex(raw, tu);
	}
...
function shader_context_parse_vertex_struct(raw: shader_context_t) {
	...
	if (ipos && !irot && !iscl) raw._instancing_type = 1;
	else if (ipos && irot && !iscl) raw._instancing_type = 2;
	else if (ipos && !irot && iscl) raw._instancing_type = 3;
	else if (ipos && irot && iscl) raw._instancing_type = 4;
}

function shader_context_delete(raw: shader_context_t) {
	if (raw._pipe_state.fragmentShader != null) shader_delete(raw._pipe_state.fragmentShader);
	if (raw._pipe_state.vertexShader != null) shader_delete(raw._pipe_state.vertexShader);
	if (raw._pipe_state.geometryShader != null) shader_delete(raw._pipe_state.geometryShader);
...
function shader_context_set_tex_params(raw: shader_context_t, g: g4_t, unit_index: i32, tex: bind_tex_t) {
