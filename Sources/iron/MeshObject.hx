...
class MeshObject extends Object {
	...
	public static var lastPipeline: PipelineState = null;
	...
	function cullMesh(context: String, camera: CameraObject, light: LightObject): Bool {
		...
		if (camera.data.raw.frustum_culling && frustumCulling) {
			...
			if (data.instanced) radiusScale *= 100;
	...
	public function getContexts(context: String, materials: Array<MaterialData>, materialContexts: Array<MaterialContext>, shaderContexts: Array<ShaderContext>) {
	...
	public function render(g: Graphics4, context: String, bindParams: Array<String>) {
		if (data == null || !data.ready) return; // Data not yet streamed
		...
		// Render mesh
		for (i in 0...data.indexBuffers.length) {

			var mi = data.materialIndices[i];
			...
			// VB / IB
			if (data.instancedVB != null) {
				g.setVertexBuffers([data.get(elems), data.instancedVB]);
			}
			else {
				g.setVertexBuffer(data.get(elems));
			}

			g.setIndexBuffer(data.indexBuffers[i]);

			// Draw
			if (data.instanced) {
				g.drawIndexedVerticesInstanced(data.instanceCount, 0, -1);
			}
			else {
	...
	function validContext(mats: Array<MaterialData>, context: String): Bool {
