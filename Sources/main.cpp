...
namespace {
	...
	int krom_get_sample_rate_fast(Local<Object> receiver) {
		kinc_log(KINC_LOG_LEVEL_INFO, "Samples per second: %d Hz.", kinc_a2_samples_per_second);	
		return kinc_a2_samples_per_second;
	}

	void krom_get_sample_rate(const FunctionCallbackInfo<Value> &args) {
		HandleScope scope(args.GetIsolate());
		args.GetReturnValue().Set(Int32::New(isolate, krom_get_sample_rate_fast(args.This())));
	}
	...
	void start_v8(char *krom_bin, int krom_bin_size) {
		...
		SET_FUNCTION(krom, "getSampleRate", krom_get_sample_rate);
		// #endif
