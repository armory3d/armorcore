...
function loadProject(directory, korefile = null) {
	...
	if (directory.endsWith("armorcore")) {
		let ___dirname = __dirname;
		__dirname = path_resolve(__dirname, "..");
...
function make_armorcore() {
	...
	class Project2 {
		...
		addProject(projectDir) {
			let project = loadProject2(projectDir, 'project.js');
	...
	function loadProject2(from, projectfile) {
	...
	function exportArmorCoreProject(options) {
		...
		if (fs_exists(path_join(options.from, 'project.js'))) {
			project = loadProject2(options.from, 'project.js');
		}
