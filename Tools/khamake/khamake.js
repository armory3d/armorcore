...
class AssetConverter {
	...
	watch(match, temp, options) {
		return new Promise((resolve, reject) => {
			let ready = false;
			let files = [];
			this.watcher = chokidar.watch(match, { ignored: /[\/\\]\.(git|DS_Store)/, persistent: false });
			const onFileChange = (file) => {
				...
				console.log('Reexporting ' + outPath + fileinfo.ext);
				switch (fileinfo.ext) {
					case '.png':
					case '.jpg':
					case '.jpeg':
...
class ShaderCompiler {
	...
	watch(match, options, recompileAll) {
		return new Promise((resolve, reject) => {
			let shaders = [];
			let ready = false;
			this.watcher = chokidar.watch(match, { ignored: /[\/\\]\.git/, persistent: false });
